      ...
      <div className={styles.dialog} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.body}>
          ...
          <p>
            ...
            保存先は2つあり、ワークスペースパネル左上の<b>保存先セレクタ</b>
            （<b>💾 ローカル</b> / <b>☁ アカウント名</b>）で切り替えます。
          </p>
          <ul>
            <li>
              <b>💾 ローカル</b> — このブラウザの内部ストレージ（IndexedDB）に保存します。
              PC上のファイルとしては作られません。はじめはこちらが選ばれています。
            </li>
            <li>
              <b>☁ Googleドライブ</b> — Googleアカウントに保存します。別のPC・別のブラウザからでも、
              同じアカウントでログインすれば同じ図を開けます。
            </li>
          </ul>
          <p className={styles.caution}>
            ⚠ <b>ローカル</b>のデータは「このPCのこのブラウザ」の中にだけあります。別のPC・別のブラウザでは開けず、
            ブラウザの「閲覧データ（サイトデータ）の削除」で図もすべて消えます。
            複数の環境で使うときや確実に残したいときは、Googleドライブに保存するか、下記のバックアップを取ってください。
          </p>
          <h4>Googleドライブに接続する</h4>
          <ul>
            <li>
              保存先セレクタを開いて <b>＋Googleアカウントを追加</b> を選び、Googleのログイン画面で
              アカウントを選んで許可すると、そのアカウントのドライブに切り替わります。
            </li>
            <li>
              アカウントは複数追加でき、セレクタから選ぶだけで切り替えられます。
              各アカウント行の <b>✕</b> でログアウトします（現在使っているアカウントをログアウトするとローカルに戻ります）。
            </li>
            <li>
              保存先はドライブの<b>このアプリ専用の領域</b>で、ドライブの通常のファイル一覧には出てきません。
              他のファイルと混ざらず、読み書きするのはこのアプリだけです。
            </li>
            <li>
              ログイン状態はしばらく保持されますが、期限切れなどで再ログインが必要になることがあります。
              そのときは自動でローカル表示に戻ります（図はドライブ側に残っているので、再ログインすれば開けます）。
            </li>
          </ul>
          ...
          <ul>
            ...
            <li>
              <b>多角形</b> — 頂点を順にクリックして置き、<b>最初の頂点をクリック</b>すると閉じます
              （<kbd>Enter</kbd>でも閉じられます / <kbd>Esc</kbd>で中止）。置いた後は
              <b>頂点（菱形ハンドル）</b>で1点ずつ、<b>辺（丸ハンドル）</b>で辺ごと平行移動できます。
              辺が交差する形にはできません（交差するクリック・ドラッグは無視されます）。
              全頂点がスナップ点なので、頂点ごとに「一致」拘束を張れます。
            </li>
            <li>
            ...
            <li>
              <b>半円・扇形</b> — 円弧・楕円弧のプロパティ「閉じ方」を「弓形」「扇形」にすると閉じた図形になり、
              塗り色・塗りパターンを付けられます（半円は掃引180°の弓形）。
            </li>
            <li>
          ...
          <ul>
            ...
            <li>
              <b>塗りつぶし</b>（<kbd>U</kbd>） — 線・円・円弧・楕円・多角形の辺などで囲まれた領域をクリックすると、
              その形の「塗り領域」ができます（カーソル下の領域が青くプレビューされます）。境界の線より背面に置かれ、
              色・不透明度・塗りパターンはプロパティパネルで変更できます。塗り領域を選択したまま次の領域をクリックすると
              同じ色で塗れます。補助線で囲んでも塗れる（補助線は書き出しに写らない）ので、扇形などの塗りに便利です。
              作った時点の形で固定され、境界を動かしても追従しません。わずかな隙間はつながっているとみなします。
            </li>
            <li>
          ...
          <table className={styles.table}>
            ...
            <tbody>
              <tr>
                <td>平行 / 垂直</td>
                <td>
                  ① 向きを合わせたい図形 → ② 基準にする線分・辺。
                  多角形・長方形など辺が複数ある図形は、①で<b>合わせたい辺の上</b>をクリックすると
                  その辺が基準に揃います（頂点を動かして辺の角度が変わっても平行を保ちます）
                </td>
              </tr>
              <tr>
                <td>一致</td>
