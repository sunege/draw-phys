...
describe('ラベルの共通機能(要件: LaTeX / 常時正立 / 背景 / ドラッグ当たり判定)', () => {
  ...
  it('block: 背景ONで白い矩形を敷く', () => {
    // block自体の塗りが#ffffffだと判別できないため、非白の塗りで検証する
    const base = { ...blockPlugin.defaultProps, fill: '#eeeeee' };
    const on = render(blockPlugin, { ...base, labelBg: true });
    expect(on).toContain('fill="#ffffff"');
    const off = render(blockPlugin, { ...base, labelBg: false });
