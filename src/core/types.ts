...
export interface ObjectRef {
  ...
  segIndex?: number;
  /**
   * 平行/垂直拘束で、**自オブジェクト側**のどの辺(getSegments のインデックス)を
   * 基準へ揃えるか。多角形・長方形のように辺を複数持つ図形で「この辺を平行に」を表す。
   * 省略時はローカルX軸(=従来どおりオブジェクトの向きそのもの)を揃える。
   * ソルバは解くたびに現在の辺の向きを見るので、頂点編集で辺の角度が変わっても追従する。
   */
  selfSegIndex?: number;
